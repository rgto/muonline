...
{
    /// <summary>
    /// Joystick virtual de movimento (canto inferior esquerdo), estilo MU mobile.
    /// FUNCIONAL: enquanto arrastado, move o herói continuamente na direção do analógico.
    /// Referência de comportamento: Main_JoyStickUI.lua do MU Dragon Havoc
    /// (drag -> direção -> targetCell = célula + dir -> MoveTo; soltar -> parar).
    /// </summary>
    public sealed class VirtualJoystickControl : UIControl
    {
        // Raio da base e do manípulo (proporção do MU Immortal: base 174, knob 78).
        private const float BaseRadius = 87f;
        private const float KnobRadius = 33.345f; // knob do DI-Immortal reduzido ~14.5% (era 39): -10% e depois -5%
        // Zona morta: abaixo disso não move (evita jitter ao só tocar).
        private const float DeadZone = 0.28f;

        // Arte real do joystick, extraída do MU Immortal (atlas_main: stick_bg/stick_main).
        ...
        private Vector2 _center;        // centro da base (UI virtual), fixo
        private Vector2 _knob;          // posição atual do manípulo (UI virtual)

        public VirtualJoystickControl()
        {
            Interactive = true;
            AutoViewSize = false;
            // Base ancorada no canto inferior esquerdo do espaço virtual.
            int marginX = 40;
            int marginBottom = 56;   // sobe o direcional um pouco acima da barra inferior
            int size = (int)(BaseRadius * 2f);
            ControlSize = new Point(size, size);
            ViewSize = new Point(size, size);
            X = marginX;
            Y = Constants.BASE_UI_HEIGHT - size - marginBottom;
        }
        ...
        public override async Task Load()
        {
            ...
            try { _texBase = await tl.PrepareAndGetTexture(StickBgPath); } catch { _texBase = null; }
            try { _texKnob = await tl.PrepareAndGetTexture(StickKnobPath); } catch { _texKnob = null; }
        }
        ...
        private void DriveMovement(HeroObject hero, Vector2 screenDir)
        {
            ...
            bool pathLow = hero.QueuedPathCount <= 3;
