...
{
    /// <summary>
    /// Barra inferior do HUD, layout do MU Immortal (main_mainmenuui / go_bottom):
    /// placa de rodapé + barra de EXP full-width na base + cristal de HP (esq) e MP (dir)
    /// + 5 slots de consumível ao centro. Arte real extraída (atlas_main/common).
    /// </summary>
    public sealed class BottomBarControl : UIControl
    {
        ...
        // Barras SD/AG: arte do usuário (verde = esquerda c/ ornamento à direita,
        // vinho = direita c/ ornamento à esquerda).
        ...
        // Versões CHEIAS: geradas da própria arte (a calha é clareada preservando
        ...
        // Degradê das extremidades (arte do usuário), desenhado POR CIMA da barra.
        ...
        // Tiras finas desenhadas por ÚLTIMO, tapando o vazamento dos degradês.
        ...
        // Dimensões dirigidas pelo BottomBarLayout (editor hud-edit-bottombar :5192).
        ...
        // Cópias ESPELHADAS (CPU) das artes da direita, orientadas como as da esquerda.
        // O draw da barra direita usa estas + FlipHorizontally: a amostragem PointClamp
        // fica IDÊNTICA à da esquerda (simetria exata) e a COR própria do lado direito
        // é preservada (o fill direito é vinho, o esquerdo dourado).
        private Texture2D _texBarRightA, _texBarRightFullA, _texBarOverlayRightA;
        private Texture2D _texClawLeft, _texClawRight;
        private Texture2D _texHudFont;

        // Atlas de glifos Segoe UI Bold 44px, baseline comum (bake da sessão 38a59482:
        // Interface/SkillWin/hud_font.OZP). Peso de fonte real = sem sombra/contorno.
        private const int HudFontH = 62;
        // Atlas REBAKEADO (mesma receita) com os glifos extras M a s t e r pro Master Level.
        private static readonly System.Collections.Generic.Dictionary<char, (int X, int W)> HudGlyphs = new()
        {
            ['0'] = (3, 28), ['1'] = (34, 28), ['2'] = (65, 28), ['3'] = (96, 28), ['4'] = (127, 28),
            ['5'] = (158, 28), ['6'] = (189, 28), ['7'] = (220, 28), ['8'] = (251, 28), ['9'] = (282, 28),
            ['.'] = (313, 15), ['%'] = (331, 41), ['L'] = (375, 26), ['v'] = (404, 27), ['E'] = (434, 26),
            ['X'] = (463, 32), ['P'] = (498, 30), ['M'] = (531, 45), ['a'] = (579, 27), ['s'] = (609, 22),
            ['t'] = (634, 20), ['e'] = (657, 27), ['r'] = (687, 21), [' '] = (711, 12)
        };

        private float HudTextWidth(string t, float scale)
        {
            float w = 0;
            foreach (var ch in t)
                if (HudGlyphs.TryGetValue(ch, out var g)) w += (g.W + 2) * scale;
            return w;
        }

        private void DrawHudText(SpriteBatch sb, string t, float x, float y, float scale, Color color)
        {
            if (_texHudFont == null || _texHudFont.IsDisposed) return;
            foreach (var ch in t)
            {
                if (!HudGlyphs.TryGetValue(ch, out var g)) continue;
                if (ch != ' ')
                    sb.Draw(_texHudFont,
                        new Rectangle((int)x, (int)y, (int)(g.W * scale), (int)(HudFontH * scale)),
                        new Rectangle(g.X, 0, g.W, HudFontH), color);
                x += (g.W + 2) * scale;
            }
        }
        ...
        public BottomBarControl(CharacterState state, ModernBottomHud hud = null)
        {
            ...
            ControlSize = new Point(Constants.BASE_UI_WIDTH, 110);
            ViewSize = ControlSize;
            X = 0;
            Y = Constants.BASE_UI_HEIGHT - 110;
        }
        ...
        public override async Task Load()
        {
            ...
            _texHudFont = await L("Interface/SkillWin/hud_font.OZP");
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            int scrW = Constants.BASE_UI_WIDTH;
            int scrH = Constants.BASE_UI_HEIGHT;
            ...
            // Nível/XP no canto inferior esquerdo com a fonte assada (Segoe UI Bold,
            // estilo BDM remaster): "Nv. 399" branco, "EXP" cinza menor, percentual no
            // MESMO ouro envelhecido da barra de XP. Sem sombra — peso da fonte resolve.
            {
                // 2 LINHAS. Antes do 400:  "Lv.299" / "EXP 52.4%".
                // Depois do 400 (Master):  "Lv.400  ML.95" / "Master EXP 41.8%"
                // (expRate já é o Master EXP em masterMode — barra e % contam a mesma coisa).
                ...
                float scBig = BottomBarLayout.LevelFont / 25f, scSmall = BottomBarLayout.ExpTextFont / 25f;
                float line2Y = scrH - BottomBarLayout.LevelTextBottom;   // linha de baixo (EXP)
                float line1Y = line2Y - HudFontH * scSmall - 4;     // linha de cima (Lv/ML)
                // Downscale 60->~25px precisa de bilinear (PointClamp serrilha a fonte).
                sb.End();
                ...
                DrawHudText(sb, nv, x, line1Y - HudFontH * scBig, scBig, Color.White);
                if (ml != null)
                {
                    x += HudTextWidth(nv, scBig) + 14;
                    DrawHudText(sb, ml, x, line1Y - HudFontH * scBig, scBig, Color.White);
                }
                x = BottomBarLayout.LevelTextX;
                DrawHudText(sb, expLbl, x, line2Y - HudFontH * scSmall, scSmall, new Color(190, 190, 190));
                x += HudTextWidth(expLbl, scSmall);
                DrawHudText(sb, pct, x, line2Y - HudFontH * scSmall, scSmall, new Color(198, 152, 86)); // mesmo ouro da barra de XP
                sb.End();
                                 ...
                                 float offsetX, float bottom, float wCfg, float hCfg, bool flip)
        {
            ...
            int y = Constants.BASE_UI_HEIGHT - (int)MathF.Round(bottom) - h;
