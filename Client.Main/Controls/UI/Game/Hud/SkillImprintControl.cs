...
{
    /// <summary>
    /// Skill Imprint — CASCA VISUAL (template do estilo moderno do MU oficial). Layout 100%
    /// vindo do editor hud-edit-imprint via <see cref="ImprintLayout"/>. Sem lógica de servidor
    /// (a mecânica Imprint não existe no protocolo Season 6). Arte em Interface/Imprint.
    /// </summary>
    public sealed class SkillImprintControl : UIControl
    {
        private const string Dir = "Interface/Imprint/";
        private const string OkCancelTex = "Interface/CharCreate/ok_cancel.OZT"; // botão padrão OK/Cancel
        ...
        private Texture2D _texHoleGlow;   // brilho por cima da skill (mesmo efeito da hotbar)
        ...
        // ── Rascunho local (só grava no hud/hotbar ao clicar SAVE) ──────────────────
        // Editar (arrastar/remover/reset/trocar aba) NÃO toca no hotbar de combate; só
        // altera estas cópias. Save empurra pro _hud; Cancel/Close descarta e recarrega.
        private readonly ushort?[][] _draftSets = { new ushort?[4], new ushort?[7] };
        private int _draftActiveSet = 1;   // qual set será o ativo quando salvar
        private bool _wasVisible;          // detecta abertura da tela (carrega o rascunho)

        // ── Lógica das 2 janelas ────────────────────────────────────────────────────
        // Nascem AMBAS abertas e coladas. Enquanto a 2ª (Skill Info) está aberta:
        //   • a borda/seta da 1ª janela (ExpBorder1/SideBtn1) fica OCULTA — ficaria por
        //     cima da 2ª janela, já que elas estão coladas;
        //   • só a borda/seta da 2ª (ExpBorder2/SideBtn2) aparece — é por onde se fecha.
        // Ao fechar a 2ª, sobra só a 1ª: aí a borda/seta DELA aparece pra poder reabrir.
        private bool _expandOpen = true;
        private bool _closeHover;
        private bool _pgLeftHover, _pgRightHover;   // hover nas setas de paginação (clareamento)
        private bool _resetHover;   // hover do Reset
        private bool _set2Selected;   // false=Set 1, true=Set 2   // mouse sobre o botão de fechar → usa a arte hover

        // ── Drag & drop das 2 janelas ──────────────────────────────────────────────
        // Cada janela tem um deslocamento próprio (em px de tela virtual), somado à origem
        // no cálculo dos rects. Arrasta-se pela barra de título vermelha (topo de cada uma).
        //   _mainDX/DY = janela 1 (Skill Imprint) | _infoDX/DY = janela 2 (Skill Info)
        ...
        {
            // TODOS os painéis ocupam 100% da altura da tela. O espaço virtual (720) já
            // representa a tela cheia — o UiScaler estica pra tela física do celular. Então
            // altura alvo = 720 (100%). Tudo (largura, elementos, fontes) escala pelo mesmo
            // fator. Em monitor gigante o UiScaler cuida de manter a proporção sem deformar.
            int targetH = Constants.BASE_UI_HEIGHT;                 // 100% da altura da tela
            _scale = targetH / (float)ImprintLayout.PanelH;
            _panelH = targetH;
            _panelW = (int)(ImprintLayout.PanelW * _scale);
            _panelOX = (Constants.BASE_UI_WIDTH - _panelW) / 2 + (int)(ImprintLayout.PanelX * _scale);
            _panelOY = (Constants.BASE_UI_HEIGHT - _panelH) / 2 + (int)(ImprintLayout.PanelY * _scale);
        }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            // Clique num slot com skill do set mostrado: remove (só no rascunho).
            {
                ...
                var slots = _set2Selected ? _bottomRects : _set1Rects;
                for (int i = 0; i < slots.Count; i++)
                    if (slots[i].Contains(p) && _draftSets[set][i].HasValue)
                    { _draftSets[set][i] = null; Scene?.SetMouseInputConsumed(); return; }
            }
        ...
        private void DrawSkillInfo(SpriteBatch sb, SpriteFont font)
        {
            ...
            var tip = SkillDatabase.GetSkillTooltip(selId);
            string desc = tip?.Description;
