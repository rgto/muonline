...
{
    /// <summary>
    /// Potion Imprint — configuração dos slots de poção/consumível do hotbar. Reaproveita a
    /// casca visual do estilo moderno (layout via <see cref="ImprintLayout"/>). A grade de cima
    /// mostra os consumíveis disponíveis no inventário; a linha de baixo tem 5 slots de poção.
    /// Arrastar um consumível da grade pra um slot equipa-o (persiste via ModernBottomHud).
    /// </summary>
    public sealed class PotionImprintControl : UIControl
    {
        ...
        private Texture2D _texPotionSlot;   // arte dos 5 slots de poção (mi_slot.OZP)
        ...
        private Rectangle _sideBtnRect;
        ...
        private void ComputeScale()
        {
            ...
            _panelOX = (Constants.BASE_UI_WIDTH - _panelW) / 2 + (int)(PotionLayout.PanelX * _scale);
            _panelOY = (Constants.BASE_UI_HEIGHT - _panelH) / 2 + (int)(PotionLayout.PanelY * _scale);
        }
        ...
        // Rect de um elemento da JANELA 2 (Potion Info).
