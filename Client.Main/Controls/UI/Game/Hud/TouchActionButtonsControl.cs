...
{
    /// <summary>
    /// Cluster de ação de toque (canto inferior direito), layout do MU Immortal:
    /// botão grande de ATAQUE + até 7 botões de SKILL num arco. Arte real extraída
    /// do MU Immortal (atlas_main: Attack, skill_frame, img_guang).
    /// Posições do arco vindas do prefab main_mainskillui (root anchor bottom-right).
    /// </summary>
    public sealed class TouchActionButtonsControl : UIControl
    {
        ...
        // Pilha de soquete da tela Skill Settings (aprovada pelo usuário): aro prateado
        // reconstruído da bg_allShow + disco ty_bg_skillBg + glow img_guang. Proporções
        // relativas ao ícone D=75 do settings: aro box 88, base box 80→75, glow 70.
        ...
        // ════════════════════════════════════════════════════════════════════════
        //  POSIÇÕES EXATAS DO PREFAB main_mainskillui.u3d (MU Immortal), extraídas
        //  do RectTransform de cada botão. NÃO ALTERAR à mão — vieram do jogo real.
        //   • Root Main_MainSkillUI: ancorado no canto inf-dir, 350x350, pivot centro.
        //   • BtnSkill1..7: ancorados no canto inf-dir; anchoredPos = (x pra esq, y pra cima).
        //   • BtnCommon (ataque): 142x132, centro = Zone(71,-44) dentro do root
        //     → relativo ao canto = (−175+71, +175−44) = (−104, +131).
        //  Escala global p/ caber na nossa UI virtual (o prefab foi feito p/ tela maior).
        // ════════════════════════════════════════════════════════════════════════
        private const float PrefabScale = 0.80f;

        // Tamanhos do prefab (sizeDelta) × Scale.
        ...
        // Sem a placa full-width do rodapé, o arco pode descer (mock do usuário).
        private const int BottomHudLift = 0;

        private const int SkillCount = 7;
        // anchoredPos EXATAS do prefab (x pra esquerda, y pra cima, relativas ao canto inf-dir).
        ...
        // Centro do ataque (BtnCommon) relativo ao canto inf-dir, do prefab.
        ...
        public TouchActionButtonsControl()
        {
            ...
            X = Constants.BASE_UI_WIDTH - w;
            Y = Constants.BASE_UI_HEIGHT - h;
        }
        ...
        public override async Task Load()
        {
            ...
            try { _texAttack = await tl.PrepareAndGetTexture(AttackTexPath); } catch { _texAttack = null; }
            try { _texSkillFrame = await tl.PrepareAndGetTexture(SkillFrameTexPath); } catch { _texSkillFrame = null; }
            try { _texSkillGlow = await tl.PrepareAndGetTexture(SkillGlowTexPath); } catch { _texSkillGlow = null; }
            try { _texSocketRing = await tl.PrepareAndGetTexture(SocketRingTexPath); } catch { _texSocketRing = null; }
            try { _texHoleBase = await tl.PrepareAndGetTexture(HoleBaseTexPath); } catch { _texHoleBase = null; }
            try { _texHoleGlow = await tl.PrepareAndGetTexture(HoleGlowTexPath); } catch { _texHoleGlow = null; }
            try { _texAttackRing = await tl.PrepareAndGetTexture(AttackRingTexPath); } catch { _texAttackRing = null; }
            try { _texAttackIcon = await tl.PrepareAndGetTexture(AttackIconTexPath); } catch { _texAttackIcon = null; }
        }

        private void LayoutButtons()
        {
            // Canto inferior direito da tela em coords de UI virtual, levantado acima do HUD.
            float cornerX = Constants.BASE_UI_WIDTH;
            float cornerY = Constants.BASE_UI_HEIGHT - BottomHudLift;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < _skillRects.Length && i < skills.Count && i < visible; i++)
            {
            ...
            else if (down && (_attackRect.Contains(p) || hitSkill >= 0))
            {
                Scene.SetMouseInputConsumed();
            }
        }
        ...
        {
            if (skill == null) { DoAttack(gs); return; }
            var target = FindNearestMonster(gs);
            if (target != null)
                gs.SkillController?.CastSkillOnTarget(skill, target);
        }
